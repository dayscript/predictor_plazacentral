...
class MatchPrediction extends Model
{
    ...
    public function updatePoints()
    {
        ...
        if(!in_array($match->status,['pending','PreMatch']) && $this->local_score !== null && $this->visit_score !== null){
            ...
            if( ($this->local_score == $this->visit_score) 
                && (($match->local_score+$match->local_pen_score) == ($match->visit_score+$match->visit_pen_score)) ) $new_points += 2;
            elseif( ($this->local_score > $this->visit_score) 
                    && (($match->local_score+$match->local_pen_score) > ($match->visit_score+$match->visit_pen_score)) ) $new_points += 2;
            elseif( ($this->local_score < $this->visit_score) 
                    && (($match->local_score+$match->local_pen_score) < ($match->visit_score+$match->visit_pen_score)) ) $new_points += 2;
        }
